...
class TorqueSpec : Spek (
{
    context("Run Torque") {
        ...
        val testRunFactory = mockk<TestRunFactory> {
            every { runTestSession(adbDeviceA, any(), any(), any(), any(), any(), any()) } returns Single.just(testSessionA)
            every { runTestSession(adbDeviceB, any(), any(), any(), any(), any(), any()) } returns Single.just(testSessionB)
        }
        ...
        given("completed test sessions") {
            ...
            it("Parses tests from apks and starts TestRuns on connectedDevices and writes the results") {
                ...
                verify {
                    ...
                    testRunFactory.runTestSession(adbDeviceA, any(), any(), any(), any(), any(), any())
                    testRunFactory.runTestSession(adbDeviceB, any(), any(), any(), any(), any(), any())
